...
define( 'AH_PUPPY_STORE_URL', 'http://shop.ahansson.com' );
define( 'AH_PUPPY_NAME', 'Puppy' );
...
$license_key = trim( get_option( 'ah_puppy_license_key' ) );

// setup the updater
$edd_updater = new EDD_SL_Plugin_Updater( AH_PUPPY_STORE_URL, __FILE__, array(
		'version' 	=> '1.1', 				// current version number
		'license' 	=> $license_key, 		// license key (used get_option above to retrieve from DB)
		'item_name' => AH_PUPPY_NAME, 		// name of this plugin
		'author' 	=> 'Aleksander Hansson' // author of this plugin
	)
);


function ah_puppy_license_menu() {
	add_plugins_page( 'Puppy License', 'Puppy License', 'manage_options', 'puppy-license', 'ah_puppy_license_page' );
}
add_action('admin_menu', 'ah_puppy_license_menu');

function ah_puppy_license_page() {
	$license 	= get_option( 'ah_puppy_license_key' );
	$status 	= get_option( 'ah_puppy_license_status' );
	?>
	<div class="wrap">
		<h2><?php _e('Puppy License Options'); ?></h2>
		<form method="post" action="options.php">

			<?php settings_fields('ah_puppy_license'); ?>
			...
			<table class="form-table">
				<tbody>
					<tr valign="top">
						...
						<td>
							<input id="ah_puppy_license_key" name="ah_puppy_license_key" type="text" class="regular-text" value="<?php esc_attr_e( $license ); ?>" />
							<label class="description" for="ah_puppy_license_key"><?php _e('Enter your license key'); ?></label>
						</td>
					</tr>
						...
						<tr valign="top">
							...
							<td>
								<?php if( $status !== false && $status == 'valid' ) { ?>
									...
									<?php wp_nonce_field( 'ah_puppy_nonce', 'ah_puppy_nonce' ); ?>
									<input type="submit" class="button-secondary" name="ah_puppy_license_deactivate" value="<?php _e('Deactivate License'); ?>"/>
								<?php } else {
									wp_nonce_field( 'ah_puppy_nonce', 'ah_puppy_nonce' ); ?>
									<input type="submit" class="button-secondary" name="ah_puppy_license_activate" value="<?php _e('Activate License'); ?>"/>
								<?php } ?>
							</td>
						</tr>
					<?php } ?>
...
function ah_puppy_register_option() {
	// creates our settings in the options table
	register_setting('ah_puppy_license', 'ah_puppy_license_key', 'ah_puppy_sanitize_license' );
}
add_action('admin_init', 'ah_puppy_register_option');

function ah_puppy_sanitize_license( $new ) {
	$old = get_option( 'ah_puppy_license_key' );
	if( $old && $old != $new ) {
		delete_option( 'ah_puppy_license_status' ); // new license has been entered, so must reactivate
	}
	return $new;
}

function ah_puppy_activate_license() {

	// listen for our activate button to be clicked
	if( isset( $_POST['ah_puppy_license_activate'] ) ) {

		// run a quick security check
	 	if( ! check_admin_referer( 'ah_puppy_nonce', 'ah_puppy_nonce' ) )
			return; // get out if we didn't click the Activate button

		// retrieve the license from the database
		$license = trim( get_option( 'ah_puppy_license_key' ) );
		...
		$api_params = array(
			...
			'license' 	=> $license,
			'item_name' => urlencode( AH_PUPPY_NAME ) // the name of our product in EDD
		);

		// Call the custom API.
		$response = wp_remote_get( add_query_arg( $api_params, AH_PUPPY_STORE_URL ), array( 'timeout' => 15, 'sslverify' => false ) );
		...
		update_option( 'ah_puppy_license_status', $license_data->license );

	}
}
add_action('admin_init', 'ah_puppy_activate_license');
...
function ah_puppy_deactivate_license() {

	// listen for our activate button to be clicked
	if( isset( $_POST['ah_puppy_license_deactivate'] ) ) {
		...
		if( $license_data->license == 'deactivated' )
			delete_option( 'ah_puppy_license_status' );

	}
}
add_action('admin_init', 'ah_puppy_deactivate_license');
...
function ba_slabtext_check_license() {
	...
	$api_params = array(
		...
		'license' => $license,
		'item_name' => urlencode( AH_PUPPY_NAME )
	);
